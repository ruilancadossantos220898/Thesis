...
function [mesh] = Circular_Mesh_with_baricenter(N,R,Xc)
...
mesh.p=p';
mesh.t=t';
Nv = max(size(mesh.p)); % Numero total de pontos (ou vertices) da mesh 
Nt = max(size(mesh.t)); % Numero total de triangulos
...
for t=1:Nt
mesh.p = [mesh.p ; (mesh.p(mesh.t(t,1),:) + mesh.p(mesh.t(t,2),:) + mesh.p(mesh.t(t,3),:))/3];
end 


mesh.ba=(1:Nt)+Nv;

mesh.ele = [mesh.t , mesh.ba'];
