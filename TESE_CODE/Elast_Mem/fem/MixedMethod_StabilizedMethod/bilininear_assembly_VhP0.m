function [B] = bilininear_assembly_VhP0(fun,mesh)
...
for j=1:4
    ...
    v=repmat(Psi_cell{j},Nt,1);
    % grad_u = invA^T*gradhat_Psi
    vx=invAt{1,1}*gradhat_Psi_cell{j}(1,:)+invAt{1,2}*gradhat_Psi_cell{j}(2,:);
    vy=invAt{2,1}*gradhat_Psi_cell{j}(1,:)+invAt{2,2}*gradhat_Psi_cell{j}(2,:);

    B=B+sparse(1:Nt,mesh.ele(j,ele_ind),(fun(u,v,ux,uy,vx,vy,x,y,lamb,h)*Wq').*abs(detA),Nt,Np);
