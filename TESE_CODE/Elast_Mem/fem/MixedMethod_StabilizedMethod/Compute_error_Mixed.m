function [e_H1,e_L2,e_inf,H1K,L2K] = Compute_error_Mixed(uh,mesh,u,ux,uy)
...
for i=1:4
    ...
    uhval=uhval+bsxfun(@times,uh(mesh.ele(i,:)),Psival);
    % add contribution to uhx
    uhxval=uhxval+bsxfun(@times,uh(mesh.ele(i,:)),Psixval);
    % add contribution to uhy
    uhyval=uhyval+bsxfun(@times,uh(mesh.ele(i,:)),Psiyval);
end
...
% global inf norm
e_inf=norm(uh-u(mesh.p(1,:)',mesh.p(2,:)'),inf);
